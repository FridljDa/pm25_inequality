...
gs <- append(plots, list(t1, legend_plot, t2))
...
g_combined <- grid.arrange(
  grobs = gs,
  widths = c(0.1, figure_width),
  heights = c(0.1, blank_space, figure_hight, 0.4),
  layout_matrix = lay
)

as_ggplot(g_combined)

ggsave(file.path("data/16_figures", paste0(methodI,"-",scenarioI), "figure5_timeseries.png"), dpi = 300, g_combined, height = 4, width = 8)
