...
#'
add_social_vuln_index <- function(df, FIPS.code.column = "FIPS.code", path_to_svi = "data/08_svi/svi_lookup_table.csv", silent = TRUE) {
  # Check if FIPS.code.column is present in df
  if (!FIPS.code.column %in% names(df)) {
    stop(paste("Error: The specified FIPS code column", FIPS.code.column, "is not present in the dataframe."))
  }

  # Add svi_bin column using FIPS code column
  df$svi_bin <- df[, FIPS.code.column]
  ...
  findreplace_df <- data.frame(
    ...
    to = svi_lookup_table$RPL_THEMES_bins
  )

  # Replace the svi_bin values using the replace_values function
  df_replaced <- df %>%
    replace_values(findreplace_df, silent = silent)
...
add_rural_urban_class <- function(df,
  ...
  findreplace_df <- data.frame(
    ...
    fromYear = rural_urban_class_df$Year
  )
  ...
  df_replaced <- purrr::map_dfr(
    ...
    function(year) {
      ...
      df_i %>%
        replace_values(findreplace_df_i, silent = silent)
    }
  )
