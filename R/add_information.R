...
#' result_df <- add_social_vuln_index(example_df, "FIPS.code", "Year", "data/08_svi/svi_lookup_table.csv")
#' print(result_df)
#' }
add_social_vuln_index <- function(df, FIPS.code.column = "FIPS.code", Year.colum = "Year", path_to_svi = "data/08_svi/svi_lookup_table.csv", silent = TRUE) {
...
#' print(result_df)
#' }
add_social_vuln_index_new <- function(df,
  ...
  df_replaced <- df_replaced %>%
    mutate(across(all_of(replace_column_names), ~ replace_na(.x, "Unknown") %>% as.factor()))
