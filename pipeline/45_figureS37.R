...
all_burd <- all_burd %>%
  select(Year, Ethnicity, value, svi_bin)
...
attr_burd <- attr_burd %>%
  left_join(all_burd %>% mutate(Year = as.integer(Year)),
    by = c("Year", "Ethnicity", "svi_bin")
  )

attr_burd1 <- attr_burd %>%
  filter(measure3 %in% c("prop. of overall burden")) %>%
  select(Year, Ethnicity, measure3, mean, value, svi_bin) %>%
  mutate(xlab = "%")
...
attr_burd3 <- attr_burd3 %>%
  ...
  select(Year, Ethnicity, svi_bin, mean = difference, value) %>%
  mutate(
...
attr_burd_average <- attr_burd_combined %>%
  group_by(Ethnicity, measure3, svi_bin, xlab) %>%
  summarise(
    mean = mean(mean),
    value = mean(value)
  )
