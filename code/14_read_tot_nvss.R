...
if (rlang::is_empty(args)) {
  ...
  totalBurdenDir <- "/Volumes/fridljand/R/HIGH/data/08_total_burden"
  totalBurdenParsedDir <- "/Volumes/fridljand/R/HIGH/data/09_total_burden_parsed"
  
  totalBurdenDir <- "/g/huber/users/fridljand/R/HIGH/data/08_total_burden"
  totalBurdenParsedDir <- "/g/huber/users/fridljand/R/HIGH/data/09_total_burden_parsed"
}
...
if (!file.exists(totalBurdenParsedDir)) {
  ...
  total_burden <- fread(totalBurdenDir, nrow=40000) #TODO , nrow=40000
  ...
  suppressWarnings(
    total_burden <- total_burden %>%
      ...
      mutate(min_age = replace_na(min_age, "Unknown"))
  )
