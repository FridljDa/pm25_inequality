...
attr_burd <- attr_burd %>%
  filter(
    Gender.Code == "All genders" & measure1 == "Deaths" & measure2 == "age-adjusted rate per 100,000" & method == "di_gee" &
      attr == "attributable" &
      source == "National Vital Statistics System" & agr_by == "county" &
...
```
```{r}
g7 <- ggplot(attr_burd_pm_weighted_county, aes(number_census_tracts)) +
  geom_histogram(bins = 50) +
  xlab("Number of census tract per county")

#sum(attr_burd_pm_weighted_county$number_census_tracts <= 300)/nrow(attr_burd_pm_weighted_county)
ggsave( file.path(figuresDir, paste0(methodI,"-",scenarioI), "figureS32_extra_hist.png"), g7, dpi = 300, height = 6, width = 8)
```

```{r}
attr_burd_pm_weighted_county %>%
  filter(state != 2 & state != 6) %>%
  ggplot(aes(x = rank(number_census_tracts, ties.method = "random"), y=pm_weighted_stddev)) +
  geom_point()
```

```{r, eval = FALSE}
attr_burd_pm_weighted_county_filtered <- attr_burd_pm_weighted_county %>%
  filter(state != 2 & state != 6) %>%
  mutate(pm_weighted_stddev_quantile = ntile(pm_weighted_stddev, 5),
         pm_weighted_stddev_quantile = as.factor(pm_weighted_stddev_quantile))

g3<-ggplot(attr_burd_pm_weighted_county_filtered,
           aes(x=pm_weighted_stddev_quantile, y=mean/100)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L))+
  geom_boxplot() +
  xlab("Quintiles of population-weighted std of PM2.5") +
...
```{r}
g5 <- attr_burd_pm_weighted_county %>%
  mutate(number_census_tracts_quantile = ntile(number_census_tracts, 4) %>% as.factor) %>%
  group_by(number_census_tracts_quantile) %>%
  mutate(
         pm_weighted_stddev_quantile = ntile(pm_weighted_stddev, 5) %>% as.factor)%>%
  ggplot(
           aes(x=pm_weighted_stddev_quantile, y=mean/100)) +
  ...
  xlab("Quintiles of population-weighted std of PM2.5") +
  ylab("Percent of all-cause mortality\n attributable to PM2.5") +
  facet_wrap(vars(number_census_tracts_quantile))

g5
ggsave(file.path(figuresDir, paste0(methodI,"-",scenarioI), "figureS32_extra.png"), dpi = 300, g5, height = 6, width = 8)
```

```{r}
# Load the GGally package
#hist(attr_burd_pm_weighted_county$number_census_tracts)
...
g_pais <- ggpairs(attr_burd_pm_weighted_county %>%
          transmute(number_census_tracts, number_census_tracts_rank = rank(number_census_tracts), pm_weighted_mean, pm_weighted_variance, pm_weighted_stddev, mean),
...
```{r}
ggsave(file.path(figuresDir, paste0(methodI,"-",scenarioI), "figureS32.png"), dpi = 300, g_combined, height = 6, width = 8)
```
