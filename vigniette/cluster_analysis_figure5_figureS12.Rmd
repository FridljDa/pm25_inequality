...
```{r}
counties_unique <- unique(data_25$county)
sf_data_25_counties <- unique(sf_data_25$county_fips)
```
...
```{r}
sf_data_25 %>%
  filter(cat == "Black American minus NH White")
```
...
sf_data_25_map_notsf_plotted_upper <- sf_data_25_map_notsf %>%
  filter(cat_diff %in% c("Black American", "Hispanic or Latino White", "NH White", "American Indian or Alaskan Native", "Asian and Pacific Islander")) %>% #
  plot_usmap(data = .,
    regions = "counties",
    values = "diff",
    exclude = c("AK", "HI")
