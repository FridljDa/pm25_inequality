...
#                      ) +
 ...
 facet_wrap(~ Ethnicity, ncol = 2) +
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))# Change font size

map1
```

```{r}
ggsave(file.path(figuresDir, paste0(methodI,"-",scenarioI), "figure4.png"), dpi = 300, map1, height = 9.4, width = 8)
ggsave(file.path(figuresDir, paste0(methodI,"-",scenarioI), "figure4.pdf"), dpi = 300, map1, height = 9.4, width = 8)
```
...
```
```{r}
ggsave(file.path(figuresDir, paste0(methodI,"-",scenarioI), "figureS9.png"), dpi = 300, height = 12, width = 8)
```
