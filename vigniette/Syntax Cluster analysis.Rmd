...
```{r}
# Metadata
meta <- read_csv("../trash/county_data_25+.csv") %>%
...
data_25 <- read_csv("../trash/county_data_25+.csv") %>%
...
# 65+
data_65 <- read_csv("../trash/county_data_65+.csv") %>%
  rename(county_fips = county,
         diff = ethnicity1_minus_ethnicity2) %>%
...
"Hispanic or Latino White minus NH White"))) %>%
  ...
  facet_wrap(.~cat_diff, nrow = 2, labeller = label_wrap_gen(width=25)) +
...
```{r}
ggsave(filename = paste0("../data/16_figures/di_gee-real/map_diff_25.png"), bg = 'white', width = 15, height = 5, dpi = 300)
ggsave(filename = paste0("../data/16_figures/di_gee-real/map_diff_25.pdf"), bg = 'white', width = 15, height = 5, dpi = 300)
```
...
sf_data_65 %>%
  filter(cat_diff %in% c("Black American minus Hispanic or Latino White", "Black American minus NH White","Hispanic or Latino White minus NH White")) %>%
  filter(state_abbr != "AK", 
         state_abbr != "HI") %>%
  ggplot() +
  geom_sf(data = meta %>%
...
```{r}
ggsave(filename = paste0("../data/16_figures/di_gee-real/map_diff_65.png"), bg = 'white', width = 15, height = 5, dpi = 300)
```
...
ggsave(filename = paste0("../data/16_figures/di_gee-real/map_clust_diff_25.png"), bg = 'white', width = 10, height = 5, dpi = 300)
