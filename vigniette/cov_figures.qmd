...
attr_burden <- lapply(file_list, read_data) %>% rbindlist(use.names = T, fill = TRUE)
...
```{r}
attr_burd_filtered_dfs_star_sum_ethn_plots[[6]] <- attr_burd_filtered_dfs_star_sum_ethn_plots[[6]] + 
...
```{r}
combined_plot_ethnicity <- ggpubr::ggarrange(plotlist = attr_burd_filtered_dfs_star_sum_ethn_plots)

combined_plot_ethnicity <- combined_plot_ethnicity #+
    #theme(plot.margin = unit(c(1, 3, 2, 1), "cm"))

combined_plot_ethnicity
```

```{r}
ggsave(file.path(here("data/18_figures/di_gee-real-25"), "extended_figure7.eps"), #eps, figure_cov_ethnicity.pdf
       dpi = 300,
       plot = combined_plot_ethnicity,
       height = 220,
       width = 205,
       units = "mm")
```
...
ggsave( filename = here(paste0("data/18_figures/di_gee-real-",min_ageI,"/extended_data_figure8.eps")),
       dpi = 300,
       plot = combined_plot,
       height = 220,
       width = 205,
...
```{r}
ggsave(filename = here(paste0("data/18_figures/di_gee-real-",min_ageI,"/extended_figure9.eps")), #eps
       dpi = 300,
       plot = g,
       height = 150,
       width = 140,
...
ggsave(
  filename = here(paste0("data/18_figures/di_gee-real-",min_ageI,"/figureS8.pdf")),
  combined_plot, height = 6, width = 8
)
```
