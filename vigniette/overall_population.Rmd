...
pm_summ1 <- pm_summ %>% filter(Education == 666 & Ethnicity == "All, All Origins" & rural_urban_class == "All" & svi_bin == "All" & svi_bin1 == "All" & svi_bin2 == "All" & svi_bin3 == "All" & svi_bin4 == "All" & min_age == 25)
...
```{r, eval = FALSE}
pop <- foreach::foreach(year = 1990:2016, .combine = rbind) %do% {
  get_population_data(agr_by = "nation", year = year, pop.summary.dir = here::here("data/12_population_summary"))
}#pop.summary.dir

pop <- pop %>% filter(rural_urban_class == "666" & svi_bin == "All" & Education == "666" & Gender.Code == "A" & Race == "All")
```

```{r, eval = FALSE}
age_groups <- pop %>%
  group_by(Year) %>%
  summarise(min_year = list(min_age))
```
...
all_burden <- all_burden %>% filter(Education == 666 & Ethnicity == "All, All Origins" & rural_urban_class == "All"& svi_bin == "All" & min_age == 25)

all_burden <- all_burden %>% filter(svi_bin1 == "All" & svi_bin2 == "All" & svi_bin3 == "All" & svi_bin4 == "All" )
```
...
attr_burd <- attr_burd %>% filter(agr_by == "nation" & Education == 666 & Ethnicity == "All, All Origins" & measure3 %in% c("value","prop. of overall burden") & svi_bin == "All" & rural_urban_class == "All")
...
```{r}
attr_burd <- attr_burd %>% filter(Education == 666 & Ethnicity == "All, All Origins" & rural_urban_class == "All"& svi_bin == "All" & min_age == 25 &
                                    svi_bin1 == "All" & svi_bin2 == "All" & svi_bin3 == "All" & svi_bin4 == "All" & method == methodI &
                                    measure2 == "age-adjusted rate per 100,000" & measure1 == "Deaths" &scenario == scenarioI &measure3 == "value")
```

```{r}
g3 <- ggplot(attr_burd, aes(x = Year, y = mean)) +
    geom_line(linewidth = 1.5) +
...
```{r}
ggsave(file.path(figuresDir, paste0(methodI, "-", scenarioI, "-25"), "figure_S_attr_burd_overall.png"), dpi = 300, g3, height = 4, width = 8)
```
