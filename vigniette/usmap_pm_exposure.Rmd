...
```{r}
#exposure_pm <- rbind(exposure_pm_1990 %>% mutate(Year = 1990), 
#                     exposure_pm_2016  %>% mutate(Year = 2016))
...
```{r, eval = FALSE}
# Load necessary libraries
exposure_pm_plot_1990 <- exposure_pm_map %>%  # Assuming exposure_pm_map is your data
  filter(Year == 1990 & !(state %in% c(2, 15))) %>%
  ggplot() +
  geom_sf(aes(fill = mean)) +
  scale_fill_distiller(palette = "RdYlBu", direction = -1) +
  labs(fill = "Population-weighted mean PM2.5 exposure levels in 1990") +
  xlab("Longitude") +
  ylab("Latitude") +
  facet_wrap(vars(Ethnicity)) +
  theme_minimal() +
...
# Save the plot with increased dimensions
ggsave("figure_us_map_pm_1990.png", 
       plot = exposure_pm_plot_1990, 
       dpi = 300, 
       height = 8,  # Increased height
       width = 12)  # Increased width

# Save the plot
ggsave(filename = file.path(here::here("data/18_figures"), paste0(methodI, "-", scenarioI, "-", min_ageI), "figure_us_map_pm_1990.png"), 
       plot = exposure_pm_plot_1990,  # Fixed plot argument
       dpi = 300, 
       height = 6, 
       width = 9)

# Display the plot
exposure_pm_plot_1990
```
